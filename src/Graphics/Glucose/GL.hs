...
module Graphics.Glucose.GL where

import qualified Graphics.Glucose        as GC

#ifdef ghcjs_HOST_OS
import qualified Graphics.Glucose.WebGL  as ES
#else
import qualified Graphics.Glucose.OpenGL as ES
#endif
...
type GLMonad = GC.C GL

type Program         = GC.Program         GL
type Shader          = GC.Shader          GL
type Texture         = GC.Texture         GL
type UniformLocation = GC.UniformLocation GL

type GLclampf        = GC.GLclampf        GL
type GLfloat         = GC.GLfloat         GL
type GLenum          = GC.GLenum          GL
type GLuint          = GC.GLuint          GL
type GLint           = GC.GLint           GL
type GLintptr        = GC.GLintptr        GL
type GLbitfield      = GC.GLbitfield      GL
type GLboolean       = GC.GLboolean       GL
type GLsizei         = GC.GLsizei         GL
type GLstring        = GC.GLstring        GL
type GLptr           = GC.GLptr           GL

type BufferableData  = GC.BufferableData  GL
type ImageData       = GC.ImageData       GL

type Buffer          = GC.Buffer          GL
type Framebuffer     = GC.Framebuffer     GL
type Renderbuffer    = GC.Renderbuffer    GL
type FloatArray      = GC.FloatArray      GL
type IntArray        = GC.IntArray        GL
type UintArray       = GC.UintArray       GL
type Extension       = GC.Extension       GL


---- | A true value.
--true  :: GLboolean
--true = GLES.true :: GLES.GLboolean GL
--
---- | A false value.
--false :: GLboolean
--false = GLES.false
...
--allocFloatArray :: (Foldable t, Real f, ToJSVal f, GLMonad m) => t f -> m FloatArray
--allocFloatArray = GLES.allocFloatArray
---- | Alloc and fill an int array.
--allocIntArray   :: (Foldable t, Integral i, ToJSVal i, GLMonad m) => t i -> m IntArray
--allocIntArray = GLES.allocFloatArray
-- #else
-- | Alloc and fill a float array.
--allocFloatArray :: (Foldable t, Real f, GLMonad m) => t f -> m FloatArray
---- | Alloc and fill an int array.
--allocIntArray   :: (Foldable t, Integral i, GLMonad m) => t i -> m IntArray
-- #endif
--
