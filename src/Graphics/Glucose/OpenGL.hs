...
instance GLES OpenGL where
  ...
  glGetFramebufferAttachmentParameter target attachment pname
    | pname `elem` [GL.GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE
                   , GL.GL_FRAMEBUFFER_ATTACHMENT_TEXTURE_CUBE_MAP_FACE
                   ] =
      GLESGLenum . fromIntegral <$> getVal
    | pname == GL.GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME =
      GLESTexture . fromIntegral <$> getVal
    | pname == GL.GL_FRAMEBUFFER_ATTACHMENT_TEXTURE_LEVEL =
      GLESGLint . fromIntegral <$> getVal
    | otherwise = return GLESNone
    where getVal = liftIO $ allocaArray 1 $ \ptr -> do
  ...
  glGetTexParameter target pname
    | pname `elem` [ GL.GL_TEXTURE_MAG_FILTER
        ...
        allocaArray 1 $ \ptr -> do
           ...
           return $ GLESGLenum $ fromIntegral enum
